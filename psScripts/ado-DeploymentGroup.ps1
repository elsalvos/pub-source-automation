...
param (
    ...
    [Parameter(Mandatory=$true)][ValidateSet('true', 'false')] [string] $runAsAutoLogon,
...
if($runAsAutoLogon -eq 'false'){
    [bool]$runAsAutoLogon = $false
}else{ [bool]$runAsAutoLogon = $true}

write-host "run as autologon variable: $($runAsAutoLogon)" -ForegroundColor Green
...
function Configure-DeploymentGroupAgent
{
    ...
    param(
        ...
        [Parameter(Mandatory=$true)] $agentInstallPath,
        [Parameter(Mandatory=$false)][String][AllowEmptyString()] $deploymentAgentTags,
        [Parameter(Mandatory=$true)][bool] $runAsAutoLogon,
        [Parameter(Mandatory=$false)][String][AllowEmptyString()] $windowsLogonAccount,
        [Parameter(Mandatory=$false)][String][AllowEmptyString()] $windowsLogonPassword
        )
            write-host "run as autologon variable: $($runAsAutoLogon)" -ForegroundColor DarkRed
    ...
    else
    {
        ...
        write-host "`t running agent as service" -ForegroundColor Red
        $agentConfigArgs = "--unattended", "--url", $accountUrl, "--auth", "PAT", "--token", $personalAccessToken, "--deploymentgroup", "--projectname", $projectName, "--deploymentgroupname", $deploymentGroupName, "--agent", $agentName, "--runasservice", "--windowslogonaccount", "NT AUTHORITY\SYSTEM", "--work", "_work"
    Write-Host $agentConfigArgs
    }
    ...
    Write-Host "`t aregument: $($agentConfigArgs)" -ForegroundColor green 
...
function Install-DeploymentGroupAgent 
{
    ...
    try 
    { write-host "run as autologon variable: $($runAsAutoLogon)" -ForegroundColor DarkMagenta


        #Initial checks
        Write-Host "`t test prerequs" -ForegroundColor Red
        Test-InstallPrerequisites
    
        #Create agent directory
        Write-Host "`t create directory" -ForegroundColor Red
        $agentInstallPath = New-AgentPath $agentInstallPath
    
        #Download Agent
        Write-Host "`t downloading agent" -ForegroundColor Red
        Download-DeploymentGroupAgent $accountUrl $personalAccessToken $agentInstallPath $agentZipFile

        Write-Host "`t configuring agent" -ForegroundColor Red
        Write-Host $runAsAutoLogon
        #Configure Agent
